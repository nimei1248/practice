...
class Raopeng(Animal):
    def __init__(self,name,blood,dps):
        Animal.__init__(self,name,blood,dps)
        self.cure_num = 4

    def cure(self,target):
        target.blood += self.cure_num
        if target.blood >= target.max_blood:
           target.blood = target.max_blood

        target.tell()


rp = Raopeng('raopeng',80,3)
sf = Sunfan('sunfan',100,15)
js = Animal('jiaoshou',300,10)
