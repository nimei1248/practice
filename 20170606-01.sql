...
FROM
    ...
    WHERE
        T1.customer_id = '1000473649'
            AND T1.login_time BETWEEN '2017-05-22 12:57:36' AND '2017-06-06 12:57:36'
            AND T1.IS_WHITE = 0
    ...
    a.login_ip IN (SELECT 
        ...
        WHERE
            T2.customer_id != '1000473649'
                AND T2.login_time BETWEEN '2017-05-22 12:57:36' AND '2017-06-06 12:57:36'
                AND T2.IS_WHITE = 0
...
|  1 | PRIMARY     | T2         | ref   | idx1,i_ip_id_level_type_white_time,idx3,idx2,idx4 | i_ip_id_level_type_white_time | 195     | a.login_ip,const |    5 |   100.00 | Using where; FirstMatch(<derived2>)          |
    ...
    EXISTS( SELECT 
        ...
        WHERE
            T2.customer_id != '1000473649'
                ...
                AND T2.CUSTOMER_TYPE >= 1
                AND T2.PRODUCT_ID = 'A01'
                AND T2.login_ip = a.login_ip);
+----+--------------------+------------+-------+---------------------------------------------------+-------------------------------+---------+------------------+------+----------+----------------------------------------------+
| id | select_type        | table      | type  | possible_keys                                     | key                           | key_len | ref              | rows | filtered | Extra                                        |
+----+--------------------+------------+-------+---------------------------------------------------+-------------------------------+---------+------------------+------+----------+----------------------------------------------+
|  1 | PRIMARY            | <derived2> | ALL   | NULL                                              | NULL                          | NULL    | NULL             |  489 |   100.00 | Using where                                  |
|  3 | DEPENDENT SUBQUERY | T2         | ref   | idx1,i_ip_id_level_type_white_time,idx3,idx2,idx4 | i_ip_id_level_type_white_time | 195     | a.login_ip,const |    5 |   100.00 | Using where                                  |
|  2 | DERIVED            | T1         | range | idx1,i_ip_id_level_type_white_time,idx3,idx2,idx4 | idx2                          | 29      | NULL             |  489 |   100.00 | Using where; Using temporary; Using filesort |
+----+--------------------+------------+-------+---------------------------------------------------+-------------------------------+---------+------------------+------+----------+----------------------------------------------+
...
*************************** 1. row ***************************
  Level: Note
   Code: 1276
Message: Field or reference 'a.login_ip' of SELECT #3 was resolved in SELECT #1
*************************** 2. row ***************************
  Level: Note
   Code: 1003
Message: /* select#1 */ select `a`.`login_ip` AS `login_ip`,`a`.`count` AS `count` from (/* select#2 */ select `db1`.`t1`.`LOGIN_IP` AS `login_ip`,count(`db1`.`t1`.`LOGIN_IP`) AS `count` from `db1`.`t_log` `t1` where ((`db1`.`t1`.`IS_WHITE` = 0) and (`db1`.`t1`.`CUSTOMER_ID` = '1000473649') and (`db1`.`t1`.`LOGIN_TIME` between '2017-05-22 12:57:36' and '2017-06-06 12:57:36') and (`db1`.`t1`.`PRODUCT_ID` = 'A01') and (`db1`.`t1`.`customer_level` <> 0) and (`db1`.`t1`.`CUSTOMER_TYPE` >= 1)) group by `db1`.`t1`.`LOGIN_IP`) `a` where exists(/* select#3 */ select 1 from `db1`.`t_log` `t2` where ((`db1`.`t2`.`IS_WHITE` = 0) and (`db1`.`t2`.`CUSTOMER_ID` <> '1000473649') and (`db1`.`t2`.`LOGIN_TIME` between '2017-05-22 12:57:36' and '2017-06-06 12:57:36') and (`db1`.`t2`.`customer_level` <> 0) and (`db1`.`t2`.`CUSTOMER_TYPE` >= 1) and (`db1`.`t2`.`PRODUCT_ID` = 'A01') and (`db1`.`t2`.`LOGIN_IP` = `a`.`login_ip`)))
...
*************************** 1. row ***************************
  Level: Note
   Code: 1003
Message: /* select#1 */ select `a`.`login_ip` AS `login_ip`,`a`.`count` AS `count` from (/* select#2 */ select `db1`.`t1`.`LOGIN_IP` AS `login_ip`,count(`db1`.`t1`.`LOGIN_IP`) AS `count` from `db1`.`t_log` `t1` where ((`db1`.`t1`.`IS_WHITE` = 0) and (`db1`.`t1`.`CUSTOMER_ID` = '1000511121') and (`db1`.`t1`.`LOGIN_TIME` between '2017-05-20 15:51:43' and '2017-06-04 15:51:43') and (`db1`.`t1`.`customer_level` <> 0) and (`db1`.`t1`.`CUSTOMER_TYPE` >= 1) and (`db1`.`t1`.`PRODUCT_ID` = 'A01')) group by `db1`.`t1`.`LOGIN_IP`) `a` where exists(/* select#3 */ select 1 from `db1`.`t_log` `t2` where ((`db1`.`t2`.`IS_WHITE` = 0) and (`db1`.`t2`.`CUSTOMER_ID` <> '1000511121') and (`db1`.`t2`.`LOGIN_TIME` between '2017-05-20 15:51:43' and '2017-06-04 15:51:43') and (`db1`.`t2`.`PRODUCT_ID` = 'A01') and (`db1`.`t2`.`customer_level` <> 0) and (`db1`.`t2`.`CUSTOMER_TYPE` >= 1)))
